...
import com.oadultradeepfield.starseek.data.mapper.toDomain
import com.oadultradeepfield.starseek.data.mapper.toEntity
...
constructor(
  ...
  override suspend fun getCachedSolve(imageHash: String): Solve? =
      dao.getSolveByHash(imageHash)?.toDomain()
  ...
  override suspend fun getJobStatus(jobId: String): Result<JobStatus> = runCatching {
    ...
    when (response.status) {
      ...
      JobStatusType.SUCCESS -> JobStatus.Success(response.result!!.toDomain())
      JobStatusType.FAILED -> JobStatus.Failed(response.error ?: "Unknown error")
    }
  }

  override suspend fun saveSolve(solve: Solve): Long = dao.insert(solve.toEntity())

  override fun getAllSolves(): Flow<List<Solve>> =
      dao.getAllSolves().map { entities -> entities.map { it.toDomain() } }

  override suspend fun getSolveById(id: Long): Solve? = dao.getSolveById(id)?.toDomain()
  ...
  override suspend fun getObjectDetail(objectName: String): Result<ObjectDetail> = runCatching {
    ...
    objectDetailDao.insert(response.toEntity())
    response.toDomain()
  }
}
